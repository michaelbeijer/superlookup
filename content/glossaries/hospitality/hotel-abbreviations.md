---
title: "Abbreviations Used In Hotels"
slug: "hotel-abbreviations"
description: "Common abbreviations used in the hotel and hospitality industry."
source_lang: "abbreviation"
target_lang: "en"
tags: ["hospitality", "hotels", "abbreviations"]
last_updated: "2026-01-16"
source_url: "http://hotelstudies.in/abbreviations-used-in-hotels/"
source_retrieved: "2021-11-25"
term_count: 38
---

| Abbreviation | Full Form |
|--------------|-----------|
| ABC | Ashtray Bud vase Cruet set |
| ADR | Average Daily Rate |
| ARR | Average Room Revenue |
| AYS | At Your Service |
| BOT | Beverage Order Ticket |
| CCP | Critical Control Point |
| CRS | Central Reservations System |
| CSA | Customer Service Associate |
| DCDP | Demi Chef De Partie |
| EBT | English Breakfast Tea |
| EHP | Executive Hotelier Plan |
| EPNS | Electroplated Nickel Silver |
| FFIT | Foreigner Free Individual Traveler |
| FHRAI | Federation of Hotels & Restaurants Association of India |
| FOM | Front Office Manager |
| FSMS | Food Safety Management System |
| GDS | Global Distribution System |
| GIT | Group Inclusive Tour |
| GRE | Guest Relations Executive |
| GSA | Guest Service Associate |
| HACCP | Hazard Analysis Critical Control Point |
| HOD | Head of Department |
| HRACC | Hotel and Restaurant Approval Classification Committee |
| HVAC | Heat Ventilation Air Conditioning |
| HWC | Handle With Care |
| IDS | Internet Distribution System |
| IKT | Inter Kitchen Transfer |
| KPI | Key Performance Indicator |
| KRA | Key Responsibility Area |
| MDR | Manager Daily Report |
| MOD | Manager On Duty |
| OOO | Out Of Order |
| PCI | Pest Control Of India |
| SPATT | Special Attention |
| TCM | Tea Coffee Milk |
| USP | Unique Selling Point |
| WC | Water Closet |
| WOT | Wine Order Ticket |
