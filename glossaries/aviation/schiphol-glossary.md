---
title: Amsterdam Airport Schiphol Glossary
slug: schiphol-glossary
description: Dutch-English aviation terminology from Amsterdam Schiphol Airport
source_lang: nl
target_lang: en
term_count: 48
source_url: https://www.schiphol.nl/
author: Schiphol Group (adapted)
license: CC-BY-4.0
last_updated: 2025-12-15
tags:
  - "aviation"
  - "transport"

---

# Amsterdam Airport Schiphol Glossary

A collection of Dutch-English terms commonly used at Amsterdam Schiphol Airport.

## Terms

| Dutch | English | Notes |
|-------|---------|-------|
| aankomst | arrival | |
| aankomsthal | arrivals hall | |
| bagage | baggage, luggage | |
| bagageband | baggage carousel | also: bagagecarrousel |
| bagagedepot | left luggage office | |
| boarding | boarding | English loanword in Dutch |
| boardingpass | boarding pass | |
| check-in | check-in | |
| check-in balie | check-in desk | also: incheckbalie |
| douane | customs | |
| duty-free | duty-free | |
| gate | gate | |
| grondpersoneel | ground staff | |
| handbagage | hand luggage, carry-on | |
| inchecken | to check in | |
| instappen | to board | |
| koffer | suitcase | |
| landing | landing | |
| landingsbaan | runway | |
| lounge | lounge | |
| luchtvaartmaatschappij | airline | |
| luchthaven | airport | |
| paspoort | passport | |
| paspoortcontrole | passport control | |
| pier | pier, concourse | |
| platform | apron, tarmac | |
| ruimbagage | checked baggage | also: ingecheckte bagage |
| security | security | English loanword |
| startbaan | runway | same as landingsbaan |
| steward | flight attendant (male) | |
| stewardess | flight attendant (female) | |
| taxfree | tax-free | |
| terminal | terminal | |
| ticket | ticket | |
| toeslag | surcharge | |
| transfer | transfer | |
| transferpassagier | transfer passenger | |
| tussenkomst | layover, stopover | |
| uitgang | exit | |
| vertraging | delay | |
| vertrek | departure | |
| vertrekhal | departure hall | |
| vervoersbewijs | travel document | |
| visum | visa | |
| vliegen | to fly | |
| vliegtuig | airplane, aircraft | |
| vlucht | flight | |
| vluchtinformatie | flight information | |
